...
function primeTests() {
  ...
  console.log(isPrime(1324786323), ' -> false');
}
...
function ransomNote(magazine, message) {
  ...
  magazine.split('').forEach(el => {
    if (Object.keys(magazineCache).includes(el.toString())) magazineCache[el] += 1;
    else magazineCache[el] = 1;
  });
  
  message.split('').forEach(el => {
    if (Object.keys(messageCache).includes(el.toString())) messageCache[el] += 1;
    else messageCache[el] = 1;
  });
  
	for (let k in  messageCache) {
    if (magazineCache[k] === undefined || magazineCache[k] < messageCache[k]) return false;
  }
...
function palindrome(string) {
  let originalString = string.toLowerCase().match(/[a-z]/g).join('');
  let prospectivePalindrome = string.toLowerCase().match(/[a-z]/g).reverse().join('');

	return originalString === prospectivePalindrome;
}
...
function shuffle(array) {
  const arrayCopy = array.slice(0);
	...
	return array.map(el => {
    const randomInd = Math.floor(Math.random() * (arrayCopy.length - 1));
    return arrayCopy.splice(randomInd, 1)[0];
  });
}
